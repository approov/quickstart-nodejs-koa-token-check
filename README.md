...
This repo implements the Approov server-side request verification code in NodeJS, which performs the verification check before allowing valid traffic to be processed by the API endpoint.

This is an Approov integration quickstart example for the NodeJS Koa framework. If you are looking for another NodeJS integration you can check our list of [quickstarts](https://approov.io/docs/latest/approov-integration-examples/backend-api/), and if you don't find what you are looking for, then please let us know [here](https://approov.io/contact). Meanwhile, you can always use the framework agnostic [quickstart example](https://github.com/approov/quickstart-nodejs-token-check) for NodeJS, and you may find that's easily adaptable to your framework of choice.
...
This is a brief overview of how the Approov cloud service and the NodeJS Koa API server fit together from a backend perspective. For a complete overview of how the mobile app and backend fit together with the Approov cloud service and the Approov SDK we recommend to read the [Approov overview](https://approov.io/product) page on our website.
...
### NodeJS Koa API Server

The NodeJS Koa API server ensures that the token supplied in the `Approov-Token` header is present and valid. The validation is done by using a shared secret known only to the Approov cloud service and the NodeJS backend server.
